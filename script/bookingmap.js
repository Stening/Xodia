$(document).ready(function() {
    ...
    var canvasY = 0;
    var clicked = false;
    var seatChosenStorage = "chosen";
    var seatTakenStorage = "taken";
    var orangeBoxMade = false;
    ...
    function isSeatTaken(seatnumber) {
        if (localStorage.getItem(seatTakenStorage) !== null && localStorage.getItem(seatTakenStorage) !== "") {
            for (var i = 0; i < JSON.parse(localStorage.getItem(seatTakenStorage)).length; i++) {
                if (JSON.parse(localStorage.getItem(seatTakenStorage))[i] == seatnumber) {
                    return true;

                }
            }
        }
        if (localStorage.getItem(seatChosenStorage) !== null && localStorage.getItem(seatChosenStorage) !== "") {
            for (var j = 0; j < JSON.parse(localStorage.getItem(seatChosenStorage)).length; j++) {
                console.log(JSON.parse(localStorage.getItem(seatChosenStorage)).length);
                if (JSON.parse(localStorage.getItem(seatChosenStorage))[j] == seatnumber) {
                    return true;
    ...
    updateSeats.onclick = function() {
        localStorage.getItem(seatTakenStorage);
        localStorage.setItem(seatTakenStorage, []);
        boxnumberid = 1;
        generateAll();
    };
    /*update seats button click*/
    ...
    resetStorage.onclick = function() {
        localStorage.setItem(seatChosenStorage, []);
        boxnumberid = 1;
        generateAll();
    };
    /*reset storage button click*/
            ...
            if (clicked === false) {
                if (canvasX > boxes[i][0][0] && canvasX < boxes[i][1][0] && canvasX < boxes[i][2][0] && canvasX > boxes[i][3][0] && canvasY > boxes[i][0][1] && canvasY > boxes[i][1][1] && canvasY < boxes[i][2][1] && canvasY < boxes[i][3][1]) {
                    if (isSeatTaken(boxes[i][4]) === false) {
                        appendToStorage(seatChosenStorage, boxes[i][4]);
                    ...
                    console.log(localStorage.getItem(seatChosenStorage));
                    clicked = true;
                }
                else {
                    console.log("testing!");
